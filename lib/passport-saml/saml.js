...
SAML.prototype.validateSignature = function (fullXml, currentNode, certs, signatureRequired = true) {
  var self = this;
  var xpathSigQuery = ".//*[local-name(.)='Signature' and " +
                      "namespace-uri(.)='http://www.w3.org/2000/09/xmldsig#']";
  var signatures = xpath(currentNode, xpathSigQuery);
  // This function is expecting to validate exactly one signature, so if we find fewer
  //   than that, reject if signature is required, otherwise verify the first.
  // If signature is not required and not found, accept.
  if (signatures.length < 1)
    return signatureRequired ? false : true;
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  .then(function(certs) {
    ...
    if (self.options.cert && !validSignature) {
      throw new Error('Top level signature is not valid')
    }
    ...
    if (assertions.length == 1) {
      if (self.options.cert && !self.validateSignature(xml, assertions[0], certs, false)) {
...
// TODO REMOVE UGLY EMERGENCY FIX COMMENTING OUT VALIDATION
//          if (self.options.cert && !self.validateSignature(decryptedXml, decryptedAssertions[0], certs))
